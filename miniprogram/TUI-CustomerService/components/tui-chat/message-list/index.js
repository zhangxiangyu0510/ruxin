...
Component({
   ...
   */
  data: {
    my_header_avater: app.globalData.imageUrl + '/icons/defaultMyIcon.svg',
    avatar: '',
    userID: '',
  ...
  pageLifetimes: {
    show: function () {
      this.changeColor('my_header_avater', app.globalData.imageUrl + '/icons/defaultMyIcon.svg', app.globalData.themeColor, 'stroke')
    }
  },
  lifetimes: {
    attached() {},
    ready() {
      ...
      wx.$TUIKit.on('onMessageReceived', this.$onMessageReceived, this);
      wx.$TUIKit.on('onMessageReadByPeer', this.$onMessageReadByPeer, this);
      wx.$TUIKit.on('onMessageRevoked', this.$onMessageRevoked, this);
    },

    detached() {
      // 一定要解除相关的事件绑定
      wx.$TUIKit.off('onMessageReceived', this.$onMessageReceived);
      wx.$TUIKit.off('onMessageReadByPeer', this.$onMessageReadByPeer);
      wx.$TUIKit.off('onMessageRevoked', this.$onMessageRevoked);
    },
  },

  methods: {
    //changesvg
    changeColor(name, url, color = '#EBEBEB', type = 'fill') {
      util.svgColor(url, color, type).then((res) => {
        this.setData({
          [name]: res
        })
      })
      ...
      value.data.forEach((item) => {
        if (this.data.messageList.length > 12 && !value.data[0].isRead &&
          ...
          this.setData({
            ...
            jumpAim: this.filterSystemMessageID(value.data[value.data.length - 1].ID),
            showDownJump: true,
          });
        } else {
          this.setData({
    ...
    // 撤回消息
    revokeMessage() {
        // "TIMVideoFileElem" "TIMImageElem"
        console.log('this.data.selectedMessage====',this.data.selectedMessage);
      wx.$TUIKit.revokeMessage(this.data.selectedMessage)
        .then((imResponse) => {
          ...
          if (imResponse.data.message.from === _userInfo.uid) {
              if(imResponse.data.message.type=='TIMVideoFileElem'||imResponse.data.message.type=='TIMImageElem'||imResponse.data.message.type=='TIMCustomElem' || imResponse.data.message.type=='TIMSoundElem'){
                this.setData({
                    showName: '你',
                    isRevoke: true,
                    isRewrite: false,
    ...
    //   wx.$TUIKit.resendMessage(this.data.resendMessage)
        // .then((imResponse) => {
          this.triggerEvent('resendMessage', {
            message: this.data.resendMessage
          });
          this.setData({
            isRevoke: true,
            isRewrite: false,
    ...
    $onMessageRevoked(event) {
      if (event.data[0].from !== _userInfo.uid) {
        this.setData({
    ...
    getMemberProfile(e) {
      ...
      if (e.currentTarget.dataset.value.conversationType === 'C2C') {
        this.data.personalProfile = {
          avatar: e.currentTarget.dataset.value.avatar,
          nick: e.currentTarget.dataset.value.nick,
          userID: e.currentTarget.dataset.value.from,
