const rateApp = getApp<IAppOption>();
Component({
  /**
   * 组件的属性列表
   */
  ...
  data: {
    ...
    svgDefault: '',
    svgActive: '',
    themeColor:rateApp.globalData.themeColor
  }as{
    rates: any[],
    svgDefault: string,
    svgActive: string
   ...
   */
  methods: {
    svgData(svg:string):void{
      const svgDefault:string = encodeURIComponent(svg.replace('<svg', `<svg style="fill: ${this.data.color};"`))
      const svgActive:string = encodeURIComponent(svg.replace('<svg', `<svg style="fill: ${rateApp.globalData.themeColor};"`))
      this.setData({
        svgDefault,
        svgActive
      })
    ...
    initData():void{
      ...
      for (let i = 0; i < this.data.count; i++) {
        const halfPre:boolean = this.data.value >= i + 0.5
        let halfNext:boolean
        if (this.data.isHalf) {
